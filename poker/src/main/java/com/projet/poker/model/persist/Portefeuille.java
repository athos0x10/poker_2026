...
public class Portefeuille {
    ...
    private BigDecimal globalBalance = BigDecimal.ZERO;
     ...
     */
    public Portefeuille(BigDecimal globalBalance, Utilisateur utilisateur) {
    ...
    public void addFunds(BigDecimal amount) {
        this.globalBalance = this.globalBalance.add(amount);
    }
    ...
    public void withdrawFunds(BigDecimal amount) {
        if (amount.compareTo(this.globalBalance) > 0) {
            throw new IllegalArgumentException(
                    "Le montant à retirer dépasse le solde actuel du portefeuille.");
        }
        // Au lieu de -= :
        this.globalBalance = this.globalBalance.subtract(amount);
    }
}
