...
public interface GameNotifier {
  ...
  void sendBestCurrentCombination(long playerId, List<Card> cards,
                                  String handType);
  void broadcastFullGameInfos(List<PlayerSession> players, Table table);

  // --- MESSAGES BROADCAST (Toute la table) ---
  void broadcastBoardUpdate(List<PlayerSession> players,
                            List<Card> communityCards);
  void broadcastBetAndStackUpdate(List<PlayerSession> players, long playerId,
                                  double bet, double stack);
  ...
  void broadcastShowdown(List<PlayerSession> players, List<Card> board,
