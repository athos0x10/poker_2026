...
public class ProfilController {
  ...
  public ProfilController(SessionManager sessionManager,
  ...
  getMonProfil(@RequestHeader("Authorization") String token) {
    Long userId = sessionManager.getUserId(token);
    if (userId == null)
      return ResponseEntity.status(401).body(
          Map.of("error", "Session invalide"));
  ...
  @GetMapping("/amis")
  public ResponseEntity<?>
  getProfilsAmis(@RequestHeader("Authorization") String token) {
    Long userId = sessionManager.getUserId(token);
    if (userId == null)
      return ResponseEntity.status(401).body(
          Map.of("error", "Session invalide"));
  ...
  @GetMapping("/{id}")
  public ResponseEntity<?> getProfil(@RequestHeader("Authorization")
    ...
    if (!amitieService.sontAmis(userId, id)) {
      return ResponseEntity.status(403).body(
          Map.of("error", "Accès interdit : ce profil n'est pas accessible"));
    }
  ...
  @PatchMapping("/me/bio")
  public ResponseEntity<?>
  modifierBio(@RequestHeader("Authorization") String token,
    ...
    Utilisateur u = utilisateurService.trouverParId(userId);
    profilService.modifierBiographie(u.getProfil().getId(),
                                     request.get("biographie"));
    return ResponseEntity.ok(Map.of("message", "Biographie mise à jour"));
  }

  private ProfilResponse mapToProfilResponse(Utilisateur u,
